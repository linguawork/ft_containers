...
/*   Updated: 2022/05/06 19:41:27 by areggie          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
namespace ft
{
	...
	class vector
	{
		...
		//	typedef 	typename allocator_type::pointer 		pointer; // shortcut to pointer of std::allocator member
			...
			// typedef		reverse_iterator<const_iterator> 		const_reverse_iterator;		// пока нет реверса
		//	typedef		std::ptrdiff_t 							difference_type;
		//	typedef		std::size_t 							size_type;		
		...
		*/
		...
		//CONSTRUCTOR TO FILL VECTOR(SIZE) WITH VALUE
		...
		*/
		...
		// template <class InputIterator>
		// vector (InputIterator first, InputIterator last, const allocator_type& alloc = allocator_type());
		//BUT I have to use enable iterator class to work only with certain data_types
		//so I use enable_if class header and and is_integral struct header with integral_constant struct 
		template <class InputIterator>
		vector (InputIterator first, InputIterator last, const allocator_type& alloc = allocator_type(), typename enable_if<!is_integral<InputIterator>::value>::type* = 0) : a_llocator(alloc)
		{
			if (first > last)
		...
		//COPY CONSTRUCTOR 
		vector (const vector& x): s_ize(0), c_apacity(0)
		{
			*this = x;
		}
		...
		~vector()
		{
			for (size_type i = 0; i < s_ize; i++)
				a_llocator.destroy(f_irst + i);
			if(c_apacity)
				a_llocator.deallocate(f_irst, c_apacity);
		}
		...
		//ITERATORS
		iterator begin(){
			return (iterator(f_irst));
		}
		const_iterator begin() const{
			return (const_iterator(f_irst));
		}
		iterator end(){
			return (iterator(f_irst + s_ize));
		}
		const_iterator end() const{
			return (const_iterator(f_irst + s_ize));
		}
